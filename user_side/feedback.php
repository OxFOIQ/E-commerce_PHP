...
$error = array();
  ...
  if (count($error)===0){
    $review_request = "UPDATE user SET feedback = ? WHERE email = ? ";
    $stmt = mysqli_stmt_init($connection);
    $prepare=mysqli_stmt_prepare($stmt,$review_request) ;
        if ($prepare){
    ...
    <nav class="navbar navbar-expand-lg navbar-expand-md navbar-expand-sm fixed-top ">
        <div class="container-fluid">
          ...
          <div class="collapse navbar-collapse" id="navbarColor01">
            <ul class="navbar-nav ms-auto">
              <li class="nav-item">
                <a class="nav-link " id="navbari" href="all_reviews.php"> 
                  <i class="fas fa-house-user mx-2"></i>All_reviews
                </a>
              </li>
              <li class="nav-item">
        ...
        <div class="row d-flex justify-content-center">
            ...
            <div class="card">
                ...
                <div class="row">
                    <form action="feedback.php" method="post" class="">
                        <div class="col-lg-10">
                            <p class="fw-bold lead mb-2"><strong><?= $_SESSION['firstname'] ." ".$_SESSION['lastname'] ?></strong></p>
                            <p class="fw-bold text-muted mb-3">Customer</p>
                            <p class="text-muted fw-light mb-4">
                                <textarea name="feedbacktext" class="form-control w-100" placeholder="Write something .." rows="6"></textarea>
                                <label class="form-label" for="form4Example6">Your feedback</label>
                            </p>
                        </div>
                        <div class="container text-center">
                             <button type="submit" name="feedback" class="btn btn-dark p-3">Submit</button>
                        </div>
                    </form>
                    </div>
                </div>
